...
const thumbnailPromptInput = document.querySelector("#thumbnail-prompt-input");
...
function setThumbnailState(thumbnail, emptyMessage = "Write a prompt above, then click Generate Thumbnail.") {
  if (thumbnail?.base64) {
...
function showWorkspace(user, scripts) {
  ...
  setThumbnailState(null, "Write a prompt above, then click Generate Thumbnail.");
  }
}
...
generateThumbnailButton.addEventListener("click", async () => {
  ...
  if (!prompt) {
    setThumbnailState(null, "Write a prompt first before generating a thumbnail.");
    return;
  }
